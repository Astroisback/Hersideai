...
import { Star, MessageSquare, User, Package, Calendar, Briefcase } from "lucide-react";
        ...
        <div>
            ...
            {reviews.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm">
                    ...
                    <p className="text-gray-500">Customer reviews will appear here after they rate your services or products.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {reviews.map((review) => (
                        <div key={review.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-shadow">
                            ...
                            {review.serviceId ? (
                                <div className="flex items-center gap-2 text-sm text-pink-600 bg-pink-50 p-3 rounded-lg border border-pink-100">
                                    <Briefcase size={16} />
                                    <span className="font-medium">{review.serviceName || "Service Review"}</span>
                                    {review.bookingDate && (
                                        <>
                                            <span className="mx-1 text-pink-300">|</span>
                                            <Calendar size={14} />
                                            <span>{review.bookingDate}</span>
                                        </>
                                    )}
                                </div>
                            ) : review.productId ? (
                                <div className="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 p-3 rounded-lg">
                                    <Package size={16} />
                                    <span>Product: {review.productName || review.productId.slice(0, 8)}</span>
                                </div>
                            ) : null}
                        </div>
                    ))}
